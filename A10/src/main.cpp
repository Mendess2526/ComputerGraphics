...
tuple<float, float, float> computeNormal(int coluna, int linha)
{
    ...
    if (coluna == 0)
        coluna++;
    else if (coluna == imageWidth)
        coluna--;

    if (linha == 0)
        linha++;
    else if (linha == imageWidth)
        linha--;
    ...
    auto res = make_tuple(a, b, c);

    //Modulo do vector
    modulo = sqrtf(powf(get<0>(res), 2) + powf(get<1>(res), 2) + powf(get<2>(res), 2));

    //Vector Unitário
    get<0>(res) = get<0>(res) / modulo;
    get<1>(res) = get<1>(res) / modulo;
    get<2>(res) = get<2>(res) / modulo;
    return res;
}
...
void prepareTerrain()
{
    ...
    for (int i = 1; i < imageWidth - 2; i++) {
        for (int j = 1; j < imageWidth - 1; j++) {
            auto nor = computeNormal(i + 1, j);
            normal.push_back(get<0>(nor));
            normal.push_back(get<1>(nor));
            normal.push_back(get<2>(nor));
